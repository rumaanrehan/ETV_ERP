...
export class IndexComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('lCNumberTemplate', { static: true }) lCNumberTemplate!: TemplateRef<any>;
  @ViewChild('actionColTemplate', { static: true }) actionColTemplate!: TemplateRef<any>;
  @ViewChild(CreateComponent, { static: false }) createSidebar!: CreateComponent;

  tableDef!: DataTableDef<LetterOfCredit_IndexTableList>;
  ...
  ngOnInit(): void {
    ...
    this.tableDef = {
      tableKey: 'Admin_LetterOfCredit_IndexTable',
      columnDef: [],
      defaultSortColumn: { sortField: 'LCNo', sortOrder: 1 },
      filterForm: this.formService.createFormGroup_DataTableFilter<LetterOfCredit_IndexTableFilter>(this.pageService.getFormConfig_DataTableFilter()),
      data: [],
      totalRecords: 0,
      loading: false
    };
    this.tableDef.columnDef = [
      ...
      { data: 'LCNo',  label: 'LC No', hideVisToggle: true, filterable: true, width: "8%", customTemplate: this.lCNumberTemplate },
      { data: 'LCDate', label: 'LC Date', filterable: true },
      { data: 'IssuerBankID', label: 'IssuerBank ID', orderable: false},
      { data: 'LCAmountFC', label: 'LCAmountFC' },
      { data: 'ExpiryDate', label: 'Expiry Date', orderable: false },
  ...
  onClickEditDetails(LCID: number): void {
    try {
      if (this.createSidebar && LCID) {
        this.pageService.GetDetails(LCID)
  ...
  loadData(): void {
    try {
      const model: DataTableParams<LetterOfCredit_IndexTableFilter> = {
