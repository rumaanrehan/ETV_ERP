...
export class IndexComponent implements OnInit, OnDestroy {
  ...
  onClickPageHeaderAddButton(): void {
    if (this.createSidebar) {
      this.createSidebar.openSidebar(true, false, this.formService.createNullObject<ServiceMaster>());
    }
  }

  onClickEditDetails(ServiceID: number, ActiveStatus: boolean) {
    try {
      if (this.createSidebar && ServiceID) {
        this.pageService.GetDetails(ServiceID)
          .pipe(takeUntil(this.destroy$))
          .subscribe({
            next: (response) => {
              if (response.IsSuccess) {
