...
export class IndexComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('outSideTestMappingActiveStatusTemplate', { static: true }) outSideTestMappingActiveStatusTemplate!: TemplateRef<any>;
  @ViewChild('actionColTemplate', { static: true }) actionColTemplate!: TemplateRef<any>;

  tableDef!: IndexTableParams<OutsideTestMappingList>;
  ...
  ngOnInit(): void {
    ...
    this.tableDef.columnDef = [
      { data: 'MappingID', visible: false, orderable: false },
      { data: 'OutsideTestCode', label: 'Code', customTemplate: this.outSideLabCodeTemplate },
      { data: 'ServiceName', label: 'Service Name'},
      { data: 'ServiceCategoryName', label: 'Category' },
      { data: 'OutsideLabName', label: 'Laboratory Name' },
      { data: 'ActiveStatus', label: 'Status', cssClass: 'text-center', customTemplate: this.outSideTestMappingActiveStatusTemplate },
  ...
  onClickPageHeaderAddButton(): void {
    if (this.createSidebar) {
      this.createSidebar.openSidebar(true, false, this.formService.createNullObject<OutsideTestMapping>());
    }
  }

  onClickEditDetails(MappingID: number, ActiveStatus: boolean) {
    try {
      if (this.createSidebar && MappingID) {
        this.pageService.GetDetails(MappingID)
          .pipe(takeUntil(this.destroy$))
          .subscribe({
            next: (response) => {
              if (response.IsSuccess) {
