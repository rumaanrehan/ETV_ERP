...
export class IndexComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('containerNumberTemplate', { static: true }) containerNumberTemplate!: TemplateRef<any>;
  @ViewChild('actionColTemplate', { static: true }) actionColTemplate!: TemplateRef<any>;
  @ViewChild(CreateComponent, { static: false }) createSidebar!: CreateComponent;

  tableDef!: DataTableDef<ExportContainer_IndexTableList>;
  ...
  ngOnInit(): void {
    ...
    this.tableDef = {
      tableKey: 'Admin_ExportContainer_IndexTable',
      columnDef: [],
      defaultSortColumn: { sortField: 'ItemGroupCode', sortOrder: 1 },
      filterForm: this.formService.createFormGroup_DataTableFilter<ExportContainer_IndexTableFilter>(this.pageService.getFormConfig_DataTableFilter()),
      data: [],
      totalRecords: 0,
      loading: false
    };
    this.tableDef.columnDef = [
      ...
      { data: 'ContainerNo',  label: 'Container No', hideVisToggle: true, filterable: true, width: "8%", customTemplate: this.containerNumberTemplate },
      { data: 'ContainerType', label: 'Container Type', filterable: true },
      { data: 'ShippedDate', label: 'Shipped Date'},
      { data: 'EstimatedArrivalDate', label: 'EstimatedArrivalDate' },
  ...
  onClickPageHeaderAddButton(): void {
    if (this.createSidebar) {
      this.createSidebar.openSidebar(false, this.formService.createNullObject<ExportContainer>());
    }
  }

  onClickEditDetails(containerID: number): void {
    try {
      if (this.createSidebar && containerID) {
        this.pageService.GetDetails(containerID)
  ...
  loadData(): void {
    try {
      const model: DataTableParams<ExportContainer_IndexTableFilter> = {
  ...
  onClickDelete(row: any): void {
    try {
      ...
      this.alertService.showConfirmationWithInput({
        ...
        text: `Do you really want to ${ActionType} the "<b>${row.ContainerNo}</b>"?`,
      })
      .then(result => {
