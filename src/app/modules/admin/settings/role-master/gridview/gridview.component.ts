...
import { RoleMaster_IndexTableFilter, RoleMaster_IndexTableList } from '../role-master';
...
export class GridviewComponent implements OnInit, OnDestroy, OnChanges {
  ...
  ngOnInit(): void {
    this.tableDef = {
      ...
      rowClick: (row) => this.onClickEditDetails(row.RoleID, row.ActiveStatus),
      data: [],
      totalRecords: 0,
      loading: false
    };
  ...
  private loadData(): void {
    ...
    const model: DataTableParams<RoleMaster_IndexTableFilter> & { sortings?: any } = {
